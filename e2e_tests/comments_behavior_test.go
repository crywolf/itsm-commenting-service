...
var _ = Describe("Comments behavior", func() {
	JustBeforeEach(func() {
		msgQueue.Clear()
	})
	...
	Describe("Creation", func() {
		...
		When("when creating comment on behalf of other user (by using 'on_behalf' header)", func() {
			BeforeEach(func() {
				payload = []byte(`{
					"entity":"incident:fef298da-8ee8-4e0b-8c8f-327b17dc16bb",
					"text": "Comment On Behalf of Someone Else"
				}`)
			})
			...
			Specify("created comment should have 'created_by' field filled with correct user data", func() {
				...
				Expect(bodyMap).To(HaveKeyWithValue("text", "Comment On Behalf of Someone Else"))
	...
	Describe("Reading", func() {
		...
		When("reading comment on behalf of other user (by using 'on_behalf' header)", func() {
			BeforeEach(func() {
				payload := []byte(`{
					"entity":"incident:fef298da-8ee8-4e0b-8c8f-327b17dc16bb",
					"text": "Comment to be read by someone else"
				}`)

				uuid = createComment(payload)
			...
			Specify("comment should be marked as read by correct user", func() {
				...
				bodyMap := mapFromJSON(body)
				Expect(bodyMap).To(HaveKeyWithValue("uuid", uuid))
				Expect(bodyMap).To(HaveKeyWithValue("text", "Comment to be read by someone else"))
				Expect(bodyMap).To(HaveKeyWithValue("entity", "incident:fef298da-8ee8-4e0b-8c8f-327b17dc16bb"))
				Expect(bodyMap).To(HaveKey("created_at"))
